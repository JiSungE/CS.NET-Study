...
using System.IO;
using static chating.ChatService;
...
namespace chating
{
    ...
    class ChatService : IChat
    {
        ...
        public void whisper(int idx1, int idx2, string msg)
        {
            Number = idx1;
            SendMessage(idx2, msg, "Receive1");
        }
        ...
        private void SendMessage(int idx2, string msg, string msgType)
        {
            //동기호출
            //MyChat(idx, msg, msgType); //MyChat.Invoke(idx, msg, msgType);
            //비동기호출
            int a = 1;
            foreach (Chat handler in List.GetInvocationList())
            {
                if (a == idx2)
                {
                    handler.BeginInvoke(idx2, msg, msgType, new AsyncCallback(EndAsync), null);
                }
                a++;
            }
        }
